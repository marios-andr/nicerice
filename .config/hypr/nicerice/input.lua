...
-- Applications
hl.bind(mainMod .. " + Q", hl.dsp.exec_cmd("~/.config/ml4w/settings/terminal.sh"), { description = "Open the terminal" })
...
hl.bind(mainMod .. " + SHIFT + C", hl.dsp.exec_cmd("hyprctl activewindow | grep pid | tr -d 'pid:' | xargs kill"), { description = "Quit active window and all open instances" })
hl.bind(mainMod .. " + F", hl.dsp.window.fullscreen({ mode = "fullscreen", action = "toggle" }), { description = "Toggle Fullscreen" })
hl.bind(mainMod .. " + SUPER_L", hl.dsp.window.fullscreen({ mode = "maximized", action = "toggle" }), { description = "Toggle Maximize Window" })
...
-- Arrow Keys
hl.bind(mainMod .. " + left",  function() ArrowAction("left") end, { description = "Move focus left" })
hl.bind(mainMod .. " + right", function() ArrowAction("right") end, { description = "Move focus right" })
...
hl.bind(mainMod .. " + M", hl.dsp.exec_cmd("qs ipc call statusbar focus"), { description = "Expand statusbar and focus it for keyboard navigation" })
...
-- If in maximized, simply cycle through the workspace windows, otherwise change focus  directionally.
function ArrowAction(dir)
    local w = hl.get_active_window()
    if w ~= nil and w.fullscreen == 1 then
        hl.dispatch(hl.dsp.window.cycle_next({ next = (dir == "right" and true or false) }))
        hl.timer(function()
